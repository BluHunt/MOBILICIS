...
public class CPUGPUActivity extends AppCompatActivity {
    ...
    DeviceInfo deviceInfo = new DeviceInfo(CPUGPUActivity.this);
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDatabase.child(deviceInfo.getModel()).child("CPU & GPU")
                .child("GPU Renderer").setValue(renderer);

        TextView ver = findViewById(R.id.version);
        ver.setText("GPU Version: " + versionStr);
        mDatabase.child(deviceInfo.getModel()).child("CPU & GPU")
                .child("GPU Version").setValue(versionStr);

        TextView ven = findViewById(R.id.vendor);
        ven.setText("GPU Vendor: " + vendor);
        mDatabase.child(deviceInfo.getModel()).child("CPU & GPU")
                .child("GPU Vendor").setValue(vendor);

        egl.eglDestroySurface(display, surface);
        egl.eglDestroyContext(display, context);
        egl.eglTerminate(display);

        Flag.setCpugpuImageCheck(Boolean.TRUE);
    }
}